...
function renderClasses(){
  return `${pageHeader('Class management','클래스 관리','클래스의 영상 강의, 자료, FAQ, 운영 안내를 관리합니다.','<button class="btn primary" onclick="openClassEditor(\'create\')">+ 새 클래스 만들기</button>')}
...
function saveClassForm(event,mode){event.preventDefault();adminToast(mode==='clone'?'클래스 복제본을 저장했습니다':'클래스를 저장했습니다');setTimeout(()=>showAdminView('classes'),700);}
