...
const saleProducts=[
  ...
  {id:'prd-middle-pass',requirement:'경매 낙찰 기초반 수강 이력 필요',name:'권리분석 실전반 이용권',desc:'기초 과정을 수강한 분을 위한 심화 과정 상품',price:390000,period:'결제일로부터 120일',periodType:'결제일 기준',periodDays:120,courses:['권리분석 실전반'],extraAccess:['경매 낙찰 기초반 2개월 추가 수강'],status:'판매중',paymentCount:11},
  {id:'prd-field-package',requirement:'기존 수강생만 신청 가능',name:'땅부자 루틴클럽 8월',desc:'기존 수강생이 계속 질문, 자료, 줌모임에 참여하는 월간 스터디',price:99000,period:'2026.08.01 ~ 08.31',periodType:'지정 수강 기간 기준',periodDays:'',courses:['땅부자 루틴클럽'],extraAccess:[],status:'준비중',paymentCount:0},
...
function renderProducts(){
  ...
  <div class="product-admin-grid">${saleProducts.map((product,index)=>`<article class="sale-product-card"><div class="sale-product-top"><span>${product.courses.length}개 클래스</span><em class="${product.status==='판매중'?'open':'soon'}">${product.status}</em></div><h2>${product.name}</h2><p>${product.desc}</p><strong>${won(product.price)}</strong><small>이용 기간 · ${product.period}</small><div class="included-courses"><b>수강 조건</b><span>${product.requirement}</span></div><div class="included-courses"><b>결제 후 볼 수 있는 클래스</b>${product.courses.concat(product.extraAccess).map(course=>`<span>${course}</span>`).join('')}</div><button type="button" onclick="openProductEditor('edit','${product.id}')">상품 수정</button></article>`).join('')}</div>
  <article class="panel product-matrix"><div class="panel-head"><div><h2>상품별 수강 흐름</h2><p>누가 신청할 수 있고, 결제 후 어떤 클래스를 볼 수 있는지 확인합니다.</p></div></div><table><thead><tr><th>상품</th><th>수강 조건</th><th>결제 후 볼 수 있는 클래스</th><th>이용 기간</th><th>결제 후</th></tr></thead><tbody>${saleProducts.map(product=>`<tr><td><b>${product.name}</b></td><td>${product.requirement}</td><td>${product.courses.concat(product.extraAccess).join(' · ')}</td><td>${product.period}</td><td>내 학습에 표시</td></tr>`).join('')}</tbody></table></article>`;
}
...
function renderProductEditor(mode='create',productId=''){
  ...
  return `<form class="product-editor" onsubmit="saveProductForm(event,'${mode}')">
    ...
    <details class="product-setup-hero panel"><summary><div><span>상품 구조 안내</span><h2>결제하면 선택한 클래스가 내 학습에 보여요</h2></div><em>자세히 보기</em></summary><div class="product-setup-detail"><p>상품은 결제용 이름과 가격을 담고, 클래스는 실제로 수강생이 보는 콘텐츠입니다. 심화 과정처럼 조건이 있는 상품은 이전 수강 기록을 보고 신청 가능 여부를 확인합니다.</p><ul><li>상품 = 가격·수강 조건·노출 상태</li><li>클래스 = 영상 강의·자료·FAQ</li><li>수강 조건 = 이전 구매/수강 기록 확인</li></ul></div></details>
