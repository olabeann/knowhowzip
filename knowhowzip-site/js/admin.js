...
const students=[
  {name:'김지훈',email:'jihoon.kim@email.com',grade:'챌린저',course:'경매 낙찰 기초반',cohort:'3기',paid:'290,000원',joined:'06.29',state:'수강 중'},
  {name:'이서연',email:'seoyeon.lee@email.com',grade:'프로',course:'권리분석 실전반',cohort:'2기',paid:'390,000원',joined:'06.29',state:'수강 중'},
  {name:'박민수',email:'minsu.park@email.com',grade:'챌린저',course:'경매 낙찰 기초반',cohort:'3기',paid:'290,000원',joined:'06.28',state:'수강 중'},
  {name:'정유진',email:'yujin.jung@email.com',grade:'마스터',course:'현장 임장 마스터',cohort:'1기',paid:'450,000원',joined:'06.28',state:'수강 대기'},
  {name:'최현우',email:'hyunwoo.choi@email.com',grade:'프로',course:'권리분석 실전반',cohort:'2기',paid:'390,000원',joined:'06.27',state:'수강 중'}
];
  ...
  return `${pageHeader('2026년 7월 9일 목요일','안녕하세요, 애매모홈님 👋','강의와 상품 운영 현황을 한눈에 확인하세요.','<button class="btn ghost" onclick="adminToast(\'미리보기 페이지를 엽니다 (예시)\')">내 페이지 보기 ↗</button>')}
    ...
    <section class="dashboard-grid single">
      ...
      <article class="panel cohort-panel">
        <div class="panel-head"><div><h2>클래스 운영 현황</h2><p>수강 기간 기준 운영 상태</p></div><button class="text-btn" onclick="showAdminView('classes')">전체 보기 →</button></div>
        <div class="cohort-list">${classes.map((c,i)=>{const state=dashboardClassState(c.period);return `<button onclick="showAdminView('classes')"><span class="cohort-color" style="background:${c.color}">${i+1}</span><span class="cohort-info"><b>${c.title.split(' · ')[0]}</b><small>${c.period}</small></span><span class="cohort-state ${state.className}">${state.label}</span><strong>${c.students}명</strong></button>`;}).join('')}</div>
      </article>
    </section>
...
function renderClasses(){
  return `${pageHeader('Class management','클래스 관리','클래스 안에 영상 강의, 자료, FAQ, 선수강 조건을 관리합니다.','<button class="btn primary" onclick="openClassEditor(\'create\')">+ 새 클래스 만들기</button>')}
  ...
  return `<form class="class-editor" onsubmit="saveClassForm(event,'${mode}')">
    ...
    <div class="editor-layout">
      ...
      <div class="editor-sections">
        ...
        <section class="panel editor-section" id="editor-cohort"><div class="editor-section-head"><i>2</i><div><h2>수강 조건·일정</h2><p>선수강 강의, 수료 기준, 신청 기간과 수강 기간을 설정합니다.</p></div><span>필수</span></div><div class="prerequisite-block"><div class="prerequisite-copy"><b>선수강 조건</b><p>이 강의를 수강하기 전에 완료해야 하는 강의를 선택하세요. 상품에 함께 포함되어 권한을 보유하더라도, 조건을 충족하기 전까지 내 학습에서는 잠금 상태로 표시됩니다.</p></div><div class="prerequisite-list"><label><input type="checkbox" ${classId==='mmoh-right'?'checked':''}><span><b>경매 낙찰 기초반</b><small>완료 시 권리분석 실전반 잠금 해제</small></span></label><label><input type="checkbox" ${classId==='mmoh-field'?'checked':''}><span><b>권리분석 실전반</b><small>완료 시 현장 임장 마스터 잠금 해제</small></span></label></div><div class="editor-fields prerequisite-fields"><label>수료 기준<select><option>영상 100% 시청</option><option>영상 80% 이상 시청</option><option>관리자 수동 완료</option></select></label><label>조건 미충족 안내 문구<input value="선수강 강의를 완료하면 수강할 수 있습니다."></label></div></div><div class="editor-fields"><label>모집 상태<select><option ${course.status==='모집중'?'selected':''}>모집중</option><option ${course.status==='모집예정'?'selected':''}>모집예정</option><option>마감</option></select></label><label>모집 시작일<input type="date" value="2026-06-15"></label><label>모집 마감일<input type="date" value="2026-06-30"></label><label>수강 시작일 <em>*</em><input required type="date" value="2026-07-05"></label><label>수강 종료일 <em>*</em><input required type="date" value="2026-08-02"></label><label>판매 가격 <em>*</em><div class="input-suffix"><input required type="number" min="0" step="1000" value="${course.price||290000}"><span>원</span></div></label><label>정가<div class="input-suffix"><input type="number" min="0" step="1000" value="390000"><span>원</span></div></label><label>수강 종료 후 열람<select><option>열람 불가</option><option>30일간 가능</option><option>기간 제한 없음</option></select></label></div></section>
...
function studentGradeClass(grade){
  if(grade.includes('레전드'))return 'legend';
  if(grade.includes('마스터'))return 'master';
  if(grade.includes('프로'))return 'pro';
  if(grade.includes('챌린저'))return 'challenger';
  return 'rookie';
}
...
function studentTable(rows,compact=false){
  return `<div class="table-wrap"><table><thead><tr><th>수강생</th><th>등급</th><th>클래스</th>${compact?'':'<th>결제 금액</th>'}<th>신청일</th><th>상태</th></tr></thead><tbody>${rows.map(s=>`<tr><td><div class="student-cell"><span>${s.name[0]}</span><div><b>${s.name}</b><small>${s.email}</small></div></div></td><td><span class="student-grade ${studentGradeClass(s.grade)}">${s.grade}</span></td><td>${s.course}</td>${compact?'':`<td><b>${s.paid}</b></td>`}<td>${s.joined}</td><td><em class="table-state ${studentStateClass(s.state)}">${s.state}</em></td></tr>`).join('')}</tbody></table></div>`;
}

function renderStudents(){
  return `${pageHeader('Student management','수강생 관리','결제·수강 상태와 클래스 참여 정보를 확인합니다.','')}
  <section class="student-grade-note panel"><div><b>수강생 등급은 결제 결과로 자동 부여됩니다.</b><p>크리에이터는 등급을 확인할 수 있고, 직접 수정은 열지 않습니다. 환불, 보상 지급, 운영 예외처럼 권한 이력이 필요한 변경은 노하우집 운영자가 처리하는 구조가 안전합니다.</p></div><button type="button" onclick="adminToast('운영자에게 등급 변경 요청을 보냈습니다 (예시)')">등급 변경 요청</button></section>
  <div class="filter-bar"><div class="admin-search">⌕ <input placeholder="이름, 이메일, 등급 검색" oninput="filterStudents(this.value)"></div><select><option>전체 클래스</option><option>경매 낙찰 기초반</option><option>권리분석 실전반</option></select><select><option>전체 등급</option><option>루키</option><option>챌린저</option><option>프로</option><option>마스터</option><option>레전드</option></select><select><option>전체 상태</option><option>수강 대기</option><option>수강 중</option><option>수강 완료</option><option>기간 만료</option></select></div>
  <article class="panel full-table" id="studentTable">${studentTable(students)}</article>`;
}

function filterStudents(query){
  const q=query.trim().toLowerCase(),filtered=students.filter(s=>`${s.name} ${s.email} ${s.grade} ${s.course}`.toLowerCase().includes(q));
...
const membershipGrades=['루키','챌린저','프로','마스터','레전드'];
const saleProducts=[
  {id:'prd-intro-pass',membershipGrade:'챌린저',name:'경매 기초 클래스 이용권',desc:'경매 낙찰 기초반을 90일 동안 수강할 수 있는 상품',price:190000,period:'결제일로부터 90일',periodType:'결제일 기준',periodDays:90,courses:['경매 낙찰 기초반'],status:'판매중',paymentCount:18,recommended:'대표 상품'},
  {id:'prd-middle-pass',membershipGrade:'프로',name:'경매 기초·권리분석 이용권',desc:'기초 클래스와 권리분석 실전반을 단계별로 수강',price:390000,period:'결제일로부터 120일',periodType:'결제일 기준',periodDays:120,courses:['경매 낙찰 기초반','권리분석 실전반'],status:'판매중',paymentCount:11,recommended:'추천 상품'},
  {id:'prd-field-package',membershipGrade:'마스터',name:'경매 실전 패키지',desc:'기초·권리분석·임장을 한 번에 구성',price:790000,period:'지정 수강기간',periodType:'지정 수강 기간 기준',periodDays:'',courses:['경매 낙찰 기초반','권리분석 실전반','현장 임장 마스터'],status:'준비중',paymentCount:0,recommended:'표시 안 함'},
  {id:'prd-basic-pass',membershipGrade:'챌린저',name:'경매 낙찰 기초반 이용권',desc:'경매 낙찰 기초반 단일 클래스 수강 상품',price:290000,period:'2026.07.05 ~ 08.02',periodType:'지정 수강 기간 기준',periodDays:'',courses:['경매 낙찰 기초반'],status:'판매중',paymentCount:24,recommended:'표시 안 함'}
];
function renderProducts(){
  return `${pageHeader('Product','상품 관리','상품별 포함 클래스와 결제 후 부여할 수강생 등급을 관리합니다.','<button class="btn primary" onclick="openProductEditor(\'create\')">+ 새 상품 만들기</button>')}
  <section class="product-policy-panel panel"><div><span>상품 구조</span><h2>상품 유형 없이 자유롭게 클래스를 구성합니다</h2><p>상품은 사용자가 결제하는 단위입니다. 상품마다 포함 클래스를 1개 이상 자유롭게 선택하고, 결제 완료 시 지정한 수강생 등급과 포함 클래스 권한을 자동 부여합니다.</p></div><ul><li>상품 결제 완료 → 수강생 등급 부여</li><li>상품 결제 완료 → 포함 클래스 권한 부여</li><li>클래스 안에 여러 영상 강의와 자료를 구성</li></ul></section>
  <div class="product-admin-grid">${saleProducts.map((product,index)=>`<article class="sale-product-card"><div class="sale-product-top"><span>${product.courses.length}개 클래스</span><em class="${product.status==='판매중'?'open':'soon'}">${product.status}</em></div><h2>${product.name}</h2><p>${product.desc}</p><strong>${won(product.price)}</strong><small>이용 기간 · ${product.period}</small><div class="included-courses"><b>포함 클래스</b>${product.courses.map(course=>`<span>${course}</span>`).join('')}</div><button type="button" onclick="openProductEditor('edit','${product.id}')">상품 수정</button></article>`).join('')}</div>
  <article class="panel product-matrix"><div class="panel-head"><div><h2>상품별 권한 매트릭스</h2><p>상품 구매 시 부여되는 수강생 등급과 클래스 권한을 확인합니다.</p></div></div><table><thead><tr><th>상품</th><th>부여 등급</th><th>포함 클래스</th><th>이용 기간</th><th>결제 후 결과</th></tr></thead><tbody>${saleProducts.map(product=>`<tr><td><b>${product.name}</b></td><td>${product.membershipGrade}</td><td>${product.courses.join(' · ')}</td><td>${product.period}</td><td>수강생 등급 + 클래스 권한 부여</td></tr>`).join('')}</tbody></table></article>`;
}
...
function renderProductEditor(mode='create',productId=''){
  ...
  const current=product||{id:'',membershipGrade:'챌린저',name:'새 상품',desc:'상품에 포함된 클래스와 수강 흐름을 설명해 주세요.',price:290000,period:'결제일로부터 90일',periodType:'결제일 기준',periodDays:90,courses:['경매 낙찰 기초반'],status:'판매중',paymentCount:0,recommended:'표시 안 함'};
  return `<form class="product-editor" onsubmit="saveProductForm(event,'${mode}')">
    <div class="editor-head"><button type="button" class="editor-back" onclick="showAdminView('products')">← 상품 관리</button><div><span>${editing?'Product editing':'Product setup'}</span><h1>${editing?'상품 수정':'새 상품 만들기'}</h1><p>${locked?'결제 이력이 있는 상품은 상품 정보만 수정할 수 있습니다. 포함 클래스, 부여 등급, 가격, 이용 기간, 정책은 잠겨 있습니다.':'상품에 포함할 클래스를 선택하고, 구매 후 부여할 수강생 등급을 설정합니다.'}</p></div><div class="editor-actions"><button type="button" class="btn ghost" onclick="adminToast('\uc0c1\ud488 \ubbf8\ub9ac\ubcf4\uae30 \ud654\uba74\uc785\ub2c8\ub2e4 (\uc608\uc2dc)')">미리보기</button><button type="button" class="btn ghost" onclick="adminToast('\uc784\uc2dc \uc800\uc7a5\ud588\uc2b5\ub2c8\ub2e4')">임시 저장</button><button type="submit" class="btn primary">${editing?'변경사항 저장':'상품 등록'}</button></div></div>
    <section class="product-setup-hero panel"><div><span>상품은 결제 대상입니다</span><h2>상품 구매 결과로 등급과 클래스 권한을 부여합니다</h2><p>상품 유형을 나누지 않고, 포함 클래스를 자유롭게 구성합니다. 사용자가 결제하면 지정한 수강생 등급이 자동 부여되고, 포함 클래스가 내 학습에 표시됩니다.</p></div><ul><li>상품 = 가격·포함 클래스·부여 등급</li><li>클래스 = 영상 강의·자료·FAQ</li><li>등급 = 결제 후 자동 부여되는 수강생 상태</li></ul></section>
    ${locked?`<section class="product-lock-notice"><b>결제 이력 ${current.paymentCount}건</b><span>이미 결제가 발생한 상품이라 1. 상품 정보만 수정 가능합니다. 결제 조건 변경이 필요하면 새 상품을 생성하세요.</span></section>`:''}
    <div class="product-editor-grid"><div class="product-editor-main">
      <section class="panel product-section editable"><div class="product-section-head"><i>1</i><div><h2>상품 정보</h2><p>공개 페이지와 결제 화면에 보이는 이름, 설명, 노출 상태를 입력합니다.</p></div></div><div class="editor-fields"><label class="wide">상품명 <em>*</em><input required value="${current.name}" placeholder="예: 경매 기초 클래스 이용권, 경매 실전 패키지"></label><label class="wide">상품 소개 <em>*</em><textarea required placeholder="상품에 포함된 클래스와 수강 흐름을 설명해 주세요.">${current.desc}</textarea></label><label>상품 상태<select><option ${selectedOption('판매중',current.status)}>판매중</option><option ${selectedOption('판매예정',current.status)}>판매예정</option><option ${selectedOption('준비중',current.status)}>준비중</option><option ${selectedOption('비공개',current.status)}>비공개</option></select></label><label>추천 표시<select><option ${selectedOption('표시 안 함',current.recommended)}>표시 안 함</option><option ${selectedOption('추천 상품',current.recommended)}>추천 상품</option><option ${selectedOption('대표 상품',current.recommended)}>대표 상품</option></select></label></div></section>
      <section class="panel product-section${lockClass}"><div class="product-section-head"><i>2</i><div><h2>포함 클래스</h2><p>이 상품을 결제하면 권한이 부여될 클래스를 1개 이상 선택합니다.</p></div>${locked?'<em>수정 불가</em>':''}</div><div class="included-lecture-list"><label><input type="checkbox" ${courseChecked(current,'경매 낙찰 기초반')} ${lockAttr}><span><b>경매 낙찰 기초반</b><small>영상 강의 4개 · 선수강 없음</small></span></label><label><input type="checkbox" ${courseChecked(current,'권리분석 실전반')} ${lockAttr}><span><b>권리분석 실전반</b><small>영상 강의 4개 · 경매 낙찰 기초반 완료 필요</small></span></label><label><input type="checkbox" ${courseChecked(current,'현장 임장 마스터')} ${lockAttr}><span><b>현장 임장 마스터</b><small>영상 강의 4개 · 권리분석 실전반 완료 필요</small></span></label></div><div class="product-warning"><b>선수강 조건은 클래스에 설정됩니다.</b><p>상품에 포함되어 권한을 보유하더라도, 선수강 조건을 충족하기 전까지 해당 클래스는 내 학습에서 잠금 상태로 표시됩니다.</p></div></section>
      <section class="panel product-section${lockClass}"><div class="product-section-head"><i>3</i><div><h2>수강생 등급</h2><p>사용자가 이 상품을 결제하면 자동으로 부여할 등급을 선택합니다.</p></div>${locked?'<em>수정 불가</em>':''}</div><div class="editor-fields"><label class="wide">결제 후 부여 등급<select ${lockAttr}>${membershipGrades.map(grade=>`<option ${selectedOption(grade,current.membershipGrade)}>${grade}</option>`).join('')}</select><small>노하우집 기본 등급을 사용하고, 크리에이터는 상품별 부여 등급만 선택합니다.</small></label></div><div class="product-warning"><b>등급은 구매 결과입니다.</b><p>수동으로 등급을 나누기보다, 결제 완료 시 지정 등급을 자동 부여합니다. 운영자가 예외적으로 수동 변경할 수 있는 기능은 후속 단계로 분리합니다.</p></div></section>
      <section class="panel product-section${lockClass}"><div class="product-section-head"><i>4</i><div><h2>가격·이용 기간</h2><p>1회 결제와 상품별 이용 기간을 우선 적용합니다.</p></div>${locked?'<em>수정 불가</em>':''}</div><div class="editor-fields"><label>결제 방식<select ${lockAttr}><option>1회 결제</option><option disabled>월 정기결제 · 후속 단계</option><option disabled>연 정기결제 · 후속 단계</option></select></label><label>판매 가격 <em>*</em><div class="input-suffix"><input required type="number" min="0" step="1000" value="${current.price}" ${lockAttr}><span>원</span></div></label><label>판매 시작일<input type="date" value="2026-07-01" ${lockAttr}></label><label>판매 종료일<input type="date" value="2026-07-31" ${lockAttr}></label><label>이용 기간 기준<select ${lockAttr}><option ${selectedOption('결제일 기준',current.periodType)}>결제일 기준</option><option ${selectedOption('지정 수강 기간 기준',current.periodType)}>지정 수강 기간 기준</option><option ${selectedOption('기간 제한 없음',current.periodType)}>기간 제한 없음</option></select></label><label>이용 기간<div class="input-suffix"><input type="number" min="1" value="${current.periodDays}" ${lockAttr}><span>일</span></div></label></div></section>
      <section class="panel product-section${lockClass}"><div class="product-section-head"><i>5</i><div><h2>환불·공개 정책</h2><p>상품 결제 전 확인할 정책과 공개 상태를 설정합니다.</p></div>${locked?'<em>수정 불가</em>':''}</div><div class="editor-fields"><label class="wide">환불 안내 <em>*</em><textarea required ${lockAttr}>결제 후 수강 시작 전까지 취소할 수 있으며, 수강 시작 후에는 노하우집 운영 정책에 따라 환불됩니다.</textarea></label><label class="wide">결제 완료 안내<textarea ${lockAttr}>결제 완료 후 수강생 등급과 포함 클래스 권한이 자동 부여됩니다.</textarea></label></div><div class="publish-options"><label><input type="radio" name="product-publish" checked ${lockAttr}><span><b>판매중으로 공개</b><small>저장 후 공개 페이지에 상품이 노출됩니다.</small></span></label><label><input type="radio" name="product-publish" ${lockAttr}><span><b>비공개로 저장</b><small>관리자에서만 확인하고 나중에 공개합니다.</small></span></label></div></section>
    </div><aside class="product-editor-side"><div class="panel product-summary"><span>상품 요약</span><h3>${current.name}</h3><p>부여 등급 · ${current.membershipGrade}</p><strong>${won(current.price)}</strong><div><b>포함 클래스</b>${current.courses.map(course=>`<em>${course}</em>`).join('')}</div><small>${locked?'결제 이력이 있어 상품 정보만 수정할 수 있습니다.':'결제 완료 시 수강생 등급과 포함 클래스 권한이 부여됩니다.'}</small></div><div class="panel product-side-guide"><b>${editing?'수정 가능 범위':'생성 후 흐름'}</b>${editing?`<ol><li>상품명</li><li>상품 소개</li><li>상품 상태</li><li>추천 표시</li></ol>`:`<ol><li>상품 공개</li><li>사용자 결제</li><li>수강생 등급 자동 부여</li><li>포함 클래스 권한 부여</li><li>내 학습에서 수강</li></ol>`}</div></aside></div>
    <div class="editor-bottom-bar"><span><b>${editing?current.name:'새 상품'}</b><small>${locked?'결제 이력 상품 · 상품 정보만 수정 가능':'필수 항목을 확인한 뒤 등록하세요.'}</small></span><div><button type="button" class="btn ghost" onclick="showAdminView('products')">취소</button><button type="button" class="btn ghost" onclick="adminToast('\uc784\uc2dc \uc800\uc7a5했습니다')">임시 저장</button><button type="submit" class="btn primary">${editing?'변경사항 저장':'상품 등록'}</button></div></div>
  </form>`;
}
