...
const publicProducts=publicCreator?publicCreator.products:[
  {id:'mmoh-basic',title:'경매 낙찰 기초반 · 4주 완성',tag:'BEST',price:290000,lead:'경매가 처음인 직장인을 위한 입문 과정. 한 달이면 혼자 입찰표를 쓸 수 있습니다.',intro:'복잡한 경매 절차를 직장인 눈높이로 풀었습니다.',grad:'linear-gradient(135deg,#AFD6E7,#7DB8D4)',tags:['입문','입찰 실습','직장인 추천'],cohort:{no:3,status:'모집중',period:'2026.07.05 ~ 08.02',deadline:'2026.06.30',seats:30,enrolled:21},requirement:{label:'처음 참여 가능'},content:{videos:['경매 절차와 권리의 이해','물건 검색과 시세 분석','입찰표 작성 실습','모의 입찰 & 라이브 Q&A'],files:['주차별 강의 노트(PDF)','입찰표 작성 템플릿','물건 분석 체크리스트']},operation:{guide:'안녕하세요! 여러분 강의신청해주셔서 진심으로 감사드립니다!\n\n✅ 경매낙찰강좌 공지사항입니다.\n아래 내용 순서대로 꼭 진행해주세요.\n\n1. 탱크옥션 가입 + 정보 제출 (필수)\n경매 사이트 2개월 무료 이용을 위해 진행해주세요.\n\n▶ 탱크옥션 가입\n👉 https://www.tankauction.com/\n\n▶ 정보 제출\n👉 https://walla.my/v/cP4iFiDGfiq81DaQKoah\n\n2. OT(오리엔테이션) 안내\n일정: 7/1(수) 오후 9시 (약 20분)\n- 진행 내용\n강의 수강 방법 안내\n노션 및 강의 링크 공유\n전체 진행 방식 설명\n\nZoom 링크는 하단 줌 라이브 일정 링크 참고해주세요.\n참석이 어려운 경우 녹화본이 제공됩니다.\n\n3. 단톡방 입장\n하단 단톡방에 입장해주세요.'}},
...
function storedManagedClassImage(product){
  if(!product)return '';
...
const saleProducts=publicProducts.map(product=>({
  ...
  name:product.title,
  desc:product.lead,
  image:storedManagedClassImage(product),
  price:product.price,
...
function classImagePreviewMarkup(product){
  const image=storedManagedClassImage(product);
  return image?`<img class="editor-cover-image" src="${image}" alt="${escapeAdminText(product.name||'클래스')} 대표 이미지">`:`<span class="editor-cover-placeholder">${houseMark(72)}</span>`;
}
function openClassImagePicker(){document.getElementById('classImageInput')?.click();}
function handleClassImageUpload(input){
  const file=input.files?.[0],status=document.getElementById('classImageStatus'),form=input.closest('.product-editor');
  if(!file||!form)return;
  if(!file.type.startsWith('image/')){input.value='';if(status){status.textContent='이미지 파일만 업로드할 수 있습니다.';status.classList.add('error');}adminToast('JPG, PNG, WebP 이미지 파일을 선택해 주세요');return;}
  const sourceUrl=URL.createObjectURL(file),image=new Image();
  image.onload=()=>{
    const canvas=document.createElement('canvas'),size=720,scale=Math.max(size/image.naturalWidth,size/image.naturalHeight),width=image.naturalWidth*scale,height=image.naturalHeight*scale;
    canvas.width=size;canvas.height=size;
    const context=canvas.getContext('2d');
    context.fillStyle='#fff';context.fillRect(0,0,size,size);
    context.drawImage(image,(size-width)/2,(size-height)/2,width,height);
    const classImage=canvas.toDataURL('image/webp',.88);
    form._classImageData=classImage;
    const preview=form.querySelector('.editor-cover-preview'),oldImage=preview?.querySelector('.editor-cover-image'),placeholder=preview?.querySelector('.editor-cover-placeholder');
    placeholder?.remove();
    if(oldImage)oldImage.src=classImage;
    else preview?.insertAdjacentHTML('afterbegin',`<img class="editor-cover-image" src="${classImage}" alt="선택한 클래스 대표 이미지">`);
    if(status){status.textContent=`${file.name} · 720 × 720px 변환 완료`;status.classList.remove('error');status.classList.add('success');}
    URL.revokeObjectURL(sourceUrl);
    adminToast('대표 이미지를 720 × 720px로 맞췄습니다');
  };
  image.onerror=()=>{URL.revokeObjectURL(sourceUrl);input.value='';if(status){status.textContent='이미지를 불러오지 못했습니다.';status.classList.add('error');}adminToast('이미지를 불러오지 못했습니다');};
  image.src=sourceUrl;
}
...
function toggleRecruitmentAlways(input){
  const section=input.closest('#product-period'),block=input.closest('.recruitment-period-block'),fields=block?.querySelector('.recruitment-date-fields'),fixed=section?.querySelector('input[name="accessMode"][value="fixed"]'),immediate=section?.querySelector('input[name="accessMode"][value="immediate"]');
  ...
  if(fixed)fixed.disabled=input.checked;
  if(input.checked&&immediate){immediate.checked=true;toggleCourseAccessMode(immediate);}
}
function renderProductEditor(mode='create',productId=''){
  ...
  const recruitmentAlways=!!current.recruitmentAlways,effectiveAccessMode=recruitmentAlways?'immediate':current.accessMode,recruitmentDateAttr=locked||recruitmentAlways?'disabled':'',fixedAccessAttr=locked||recruitmentAlways?'disabled':'';
...
function validateProductPeriodSettings(form){
  ...
  if(recruitmentAlways&&mode!=='immediate'){adminToast('상시 모집은 결제 즉시 수강 방식으로 설정해 주세요');showProductEditorStep(3,true);return false;}
  return true;
}
function saveProductForm(event,mode){
  ...
  if(product){
    ...
    const classImage=form._classImageData||product.image||'';
    if(classImage){
      product.image=classImage;
      const publicProduct=publicProducts.find(item=>item.id===product.id);
      if(publicProduct)publicProduct.image=classImage;
      try{localStorage.setItem(`nhz-class-image-${product.id}`,classImage);}catch(error){adminToast('이미지는 적용했지만 브라우저 저장 공간이 부족할 수 있습니다');}
    }
    if(!priceLocked){
      ...
      product.recruitmentStart=product.recruitmentAlways?null:(form.elements.recruitmentStart?.value||'');
      product.recruitmentEnd=product.recruitmentAlways?null:(form.elements.recruitmentEnd?.value||'');
    }
  }
  clearEditorSession();
