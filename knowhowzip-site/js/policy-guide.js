(function(){
  ...
  const POLICIES=[
    {id:"admin.dashboard",pages:["creator-admin"],selector:".metric-grid.three",type:"데이터",title:"대시보드 집계 기준",summary:"대시보드의 각 지표는 다음 기준으로 집계합니다.",rules:["매출은 결제 완료 기준으로 집계합니다.","수강생은 해당 크리에이터의 강의를 수강 중인 수강생 수를 집계합니다.","클래스는 등록된 클래스 기준으로 집계합니다."],spec:"PRODUCT_SPEC §6.1 · §9"},
    ...
    {id:"admin.class-list",pages:["creator-admin"],selector:".product-admin-grid",type:"목록 UI",title:"클래스 관리 목록 정책",summary:"수강생에게 노출·판매되는 클래스의 운영 정보를 표시합니다.",rules:["클래스명, 가격, 기간, 상태, 수강 조건과 연결 콘텐츠를 표시합니다.","카드 전체를 선택하면 클래스 수정 화면으로 이동합니다.","공개와 비공개 클래스를 모두 표시합니다.","수강생 화면에서는 상품이 아니라 클래스라는 명칭을 사용합니다."],spec:"PRODUCT_SPEC §6.4 · §14.7"},
    ...
    {id:"admin.content-assets",pages:["creator-admin"],selector:"#editor-content",type:"필수 · 파일",title:"영상·자료 등록 정책",summary:"영상은 1개 이상 필수이고 제공 자료는 선택입니다.",rules:["영상 순서가 수강생 화면의 강의 순서가 됩니다.","영상 형식은 MP4, MOV, WebM이며 강의당 최대 2GB입니다.","자료는 0개 이상이며 파일당 최대 50MB입니다."],spec:"PRODUCT_SPEC §4.5~4.6 · §6.3"},
    {id:"admin.class-info",pages:["creator-admin"],selector:"#product-class-info",type:"전체 필수",title:"클래스 정보 정책",summary:"수강생 판매 화면에 표시되는 클래스 기본 정보는 전체 필수입니다.",rules:["클래스명, 한 줄 소개, 상세 소개와 대표 이미지가 필요합니다.","카테고리와 난이도를 선택하고 키워드 태그를 1개 이상 등록합니다.","상세 소개는 스마트 에디터로 작성하며 텍스트 서식, 목록, 링크와 이미지를 지원합니다.","기수는 별도 필드 없이 클래스명에 포함합니다."],spec:"PRODUCT_SPEC §6.4 · 1단계"},
    ...
    {id:"admin.refund",pages:["creator-admin"],selector:"#product-refund",type:"공통 규정",title:"환불 규정 적용 기준",summary:"확정된 공통 환불 규정을 읽기 전용으로 표시합니다.",rules:["크리에이터가 클래스별로 환불 규정을 입력하거나 수정할 수 없습니다.","전체 내용은 기본적으로 접고 자세히 보기를 선택하면 펼칩니다.","전액 환불 기간은 지정 기간 수강과 결제 즉시 수강을 구분합니다.","재생 또는 자료 다운로드 기록은 수강한 강의 콘텐츠 판단 기준으로 사용합니다."],spec:"PRODUCT_SPEC §6.4 · 7단계 / §14.6"},
    {id:"admin.student-state",pages:["creator-admin"],selector:"#studentStateSummary",type:"상태",title:"수강생 상태 분류 정책",summary:"수강생 관리에서는 결제 상태가 아니라 현재 학습 상태를 표시합니다.",rules:["전체, 수강 대기, 수강 중, 수강 종료를 같은 너비로 표시합니다.","상태 탭을 선택하면 목록도 함께 필터링합니다.","목록은 10명 단위로 페이징합니다."],spec:"PRODUCT_SPEC §6.5 · §13.2"},
    ...
    {id:"admin.channel",pages:["creator-admin"],selector:".settings-profile-form",type:"공개 반영",title:"공개 채널 정보 정책",summary:"저장한 프로필 정보는 크리에이터 공개 페이지에 반영합니다.",rules:["프로필 이미지, 이름, 핸들, 카테고리와 소개를 관리합니다.","프로필 상단 배경은 이미지가 아니라 색상으로 설정합니다.","관리자 미리보기와 공개 페이지에 같은 색을 적용합니다."],spec:"PRODUCT_SPEC §4.10 · §6.7"},
    {id:"admin.payout-settings",pages:["creator-admin"],selector:"#settingsPanel .settings-form:not(.settings-profile-form)",type:"민감정보 · 마감",title:"정산 정보 변경 정책",summary:"정산 계좌와 사업자 정보는 권한이 있는 크리에이터만 변경할 수 있습니다.",rules:["예금주, 은행, 계좌번호와 정산 이메일을 관리합니다.","정산일 7일 전까지만 화면에서 변경할 수 있습니다.","마감 이후 변경은 운영팀 확인 절차를 거칩니다.","실서비스에서는 변경 이력과 본인 확인이 필요합니다."],spec:"PRODUCT_SPEC §6.7"},
    ...
    {id:"public.creator-classes",pages:["public"],selector:"#view-creator #cs-class",type:"공개 목록",title:"크리에이터 클래스 목록 정책",summary:"해당 크리에이터가 공개한 판매 클래스만 표시합니다.",rules:["수강생 화면에서는 강의 콘텐츠를 별도 판매 단위로 노출하지 않습니다.","클래스 상태, 제목과 가격을 표시합니다.","수강 조건이 있는 클래스는 신청 전에 안내합니다."],spec:"PRODUCT_SPEC §5.3 · §14.7"},
    ...
    {id:"public.refund",pages:["public"],selector:"#view-detail #sec-refund",type:"공통 규정",title:"환불 규정 노출 기준",summary:"결제 전에 확인할 수 있도록 클래스 상세에 공통 환불 규정을 표시합니다.",rules:["모든 클래스에 동일한 규정을 읽기 전용으로 표시합니다.","전체 내용은 기본적으로 접고 자세히 보기를 선택하면 펼칩니다.","재생·다운로드에 따른 제한과 중도 해지 계산식을 함께 안내합니다.","문의 경로는 노하우집 카카오톡 채널로 통일합니다."],spec:"PRODUCT_SPEC §5.4 · §14.6"},
    ...
    {id:"public.learning",pages:["public"],selector:"#view-mypage .learning-group",type:"수강 권한 · 진도",title:"내 학습 클래스 표시 정책",summary:"구매 기록이 아니라 현재 수강 권한이 있는 클래스 목록을 표시합니다.",rules:["클래스 카드에는 상태별 영상 수 뱃지를 표시하지 않고 전체 영상 수만 표시합니다.","영상 목록에서는 재생 전 수강 전, 실제 재생 시작 후 수강 중, 마지막 구간 도달 후 수강 완료로 표시합니다.","수강 종료 클래스는 콘텐츠 접근을 제한합니다.","영상, 자료, 운영 안내와 FAQ를 클래스 안에서 제공합니다."],spec:"PRODUCT_SPEC §5.5 · §14.5"},
    ...
    {id:"public.payments",pages:["public"],selector:"#view-account .payment-history",type:"결제 이력",title:"결제 내역 표시 정책",summary:"사용자 본인의 클래스 결제·취소·환불 상태만 표시합니다.",rules:["결제일, 주문번호, 클래스, 금액과 결제 상태를 표시합니다.","결제 내역의 환불 요청 버튼은 노하우집 카카오톡 채널을 엽니다.","PG와 서버의 주문 상태를 동기화합니다.","환불 완료 시 수강 권한도 플랫폼 정책에 따라 갱신합니다."],spec:"PRODUCT_SPEC §5.6 · §14.6"},
