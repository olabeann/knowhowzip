(function(){
  ...
  const POLICIES=[
    ...
    {id:"admin.period-price",pages:["creator-admin"],selector:"#product-period",type:"가격 · 기간",title:"판매 조건 검증",summary:"가격, 모집 방식과 수강 시작 방식은 결제 전에 서버에서 다시 검증해야 합니다.",rules:["할인 가격은 선택이며 판매 가격보다 낮아야 합니다.","상시 모집은 공개 상태인 동안 기간 제한 없이 판매합니다.","날짜 지정 모집은 시작일과 종료일 안에서만 판매하며 종료일은 시작일보다 빠를 수 없습니다.","지정 기간 수강은 시작일과 종료일을 사용합니다.","결제 즉시 수강은 결제일부터 적용할 수강 가능 일수를 사용합니다."],spec:"PRODUCT_SPEC §6.4 · 4단계 / §10"},
    ...
    {id:"public.purchase-card",pages:["public"],selector:"#view-detail .buycard",type:"결제 가능 여부",title:"클래스 신청 검증",summary:"클라이언트 표시와 별개로 서버가 최종 결제 가능 여부를 판단합니다.",rules:["클래스가 공개 상태이고 날짜 지정 모집은 현재가 모집 기간 안이어야 합니다.","상시 모집은 공개 상태인 동안 기간 제한 없이 결제할 수 있습니다.","강의 콘텐츠가 1개 이상 연결되어야 합니다.","선수 조건을 충족하지 못하면 상세는 제공하되 결제를 차단합니다.","이미 유효한 권한이 있으면 중복 결제 대신 내 학습으로 안내합니다."],spec:"PRODUCT_SPEC §14.4"},
